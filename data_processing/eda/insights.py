...
class InsightsGenerator:
    ...
    def generate_summary(self):
        ...
        print(
            f"   • {lider.get('nombre')} domina con {lider.get('porcentaje')}% de la producción"
        )
    ...
    def _generate_efficiency_insights(self):
        ...
        for dept in self.df["departamento"].unique():
            ...
            consistency = (
                1 - (dept_data["produccion_mwh"].std() / avg_production)
                if avg_production > 0
                else 0
            )
            ...
            print(
                f"   • {dept}: {avg_production:.2f} MWh promedio (Consistencia: {consistency:.2f})"
            )
        ...
        most_efficient = max(
            dept_efficiency.items(), key=lambda x: x[1]["produccion_promedio"]
        )
    ...
    def _generate_risk_insights(self):
        ...
        if concentration_risk:
            for risk in concentration_risk:
                print(
                    f"   • {risk['departamento']}: {risk['porcentaje']:.1f}% - Riesgo {risk['nivel_riesgo']}"
                )
        else:
        ...
        self.insights["diversificacion_tecnologica"] = {
            ...
            "distribucion": {
                tech: float((prod / total_production) * 100)
                for tech, prod in tech_production.items()
            },
            "nivel_diversificacion": (
                "alta"
                if tech_diversity >= 3
                else "media" if tech_diversity == 2 else "baja"
            ),
        }

        print(
            f"   • Diversificación tecnológica: {self.insights['diversificacion_tecnologica']['nivel_diversificacion']}"
        )
    ...
    def _generate_correlation_insights(self):
        ...
        if correlations:
            for pair, data in correlations.items():
                ...
                print(
                    f"   • {techs}: Correlación {data['tipo']} {data['fuerza']} ({data['correlacion']:.2f})"
                )
        else:
        ...
        if len(cross_analysis.columns) == 2:  # Solo para dos tecnologías
            ...
            complementarity_score = 1 - abs(
                cross_analysis[tech_names[0]].corr(cross_analysis[tech_names[1]])
            )
            ...
            self.insights["complementariedad"] = {
                ...
                "nivel": (
                    ...
                    else "media" if complementarity_score > 0.4 else "baja"
                ),
                "tecnologias": tech_names,
            }

            print(
                f"   • Complementariedad {' + '.join(tech_names)}: {self.insights['complementariedad']['nivel']}"
            )
