...
class EnergyEDA:
    ...
    def basic_info(self):
        ...
        basic_stats = {
            ...
            "dias_totales": (pd.to_datetime(self.df['fecha'].max()) - pd.to_datetime(self.df['fecha'].min())).days,
            ...
            "porcentaje_completitud": round((1 - self.df.isnull().sum().sum() / (len(self.df) * len(self.df.columns))) * 100, 2)
        }
        ...
        produccion_stats = {
            ...
            "coeficiente_variacion": float(self.df['produccion_mwh'].std() / self.df['produccion_mwh'].mean()),
    ...
    def analyze_departments(self):
        ...
        dept_stats = self.df.groupby("departamento")["produccion_mwh"].agg([
        ...
        for dept in self.df["departamento"].unique():
            ...
            dept_detailed[dept] = {
                ...
                "porcentaje_total": float(dept_data.sum() / self.df["produccion_mwh"].sum() * 100),
        ...
        dept_ranking = {
            "ranking_produccion": dict(dept_stats['sum'].sort_values(ascending=False).head(10)),
            "ranking_eficiencia": dict(dept_stats['mean'].sort_values(ascending=False).head(10)),
            "ranking_consistencia": dict((dept_stats['mean'] / dept_stats['std']).sort_values(ascending=False).head(10)),
        ...
        self.insights["departamento_lider"] = {
            ...
            "porcentaje": float(dept_stats.loc[top_dept, 'sum'] / self.df["produccion_mwh"].sum() * 100),
            "superioridad": float(dept_stats.loc[top_dept, 'sum'] / dept_stats['sum'].nlargest(2).iloc[1])
        }
        ...
        print("=" * 50)
        print(f"Líder: {top_dept} con {self.insights['departamento_lider']['porcentaje']:.1f}% del total")

    def analyze_technologies(self):
        # Análisis detallado por tecnologías
        tech_stats = self.df.groupby("tecnologia")["produccion_mwh"].agg([
            'sum', 'mean', 'median', 'std', 'min', 'max', 'count'
        ]).round(2)
        ...
        for tech in self.df["tecnologia"].unique():
            ...
            tech_detailed[tech] = {
                ...
                "participacion_mercado": float(tech_data.sum() / self.df["produccion_mwh"].sum() * 100),
        ...
        tech_comparison = {
            "dominancia_solar": float(self.df[self.df["tecnologia"] == "Solar"]["produccion_mwh"].sum()),
            "dominancia_eolica": float(self.df[self.df["tecnologia"] == "Eólica"]["produccion_mwh"].sum()),
        ...
        self.insights["tecnologia_dominante"] = {
            ...
            "participacion": float(tech_stats.loc[lider_tech, 'sum'] / self.df["produccion_mwh"].sum() * 100),
            "ventaja_competitiva": float(tech_stats.loc[lider_tech, 'sum'] / tech_stats['sum'].sum() * 100)
        }
        ...
        print("=" * 50)
        print(f"Tecnología líder: {lider_tech} ({tech_detailed[lider_tech]['participacion_mercado']:.1f}%)")
    ...
    def analyze_temporal_patterns(self):
        ...
        monthly_production = self.df.groupby("mes")["produccion_mwh"].agg(['sum', 'mean', 'count']).round(2)
        ...
        self.insights["estacionalidad"] = {
            ...
            "diferencia_absoluta": float(monthly_production.loc[best_month, "sum"] - monthly_production.loc[worst_month, "sum"]),
            "diferencia_relativa": float((monthly_production.loc[best_month, "sum"] / monthly_production.loc[worst_month, "sum"] - 1) * 100),
            "variabilidad_mensual": float(monthly_production["sum"].std() / monthly_production["sum"].mean()),
        ...
        print(f"Variabilidad estacional: {self.insights['estacionalidad']['variabilidad_mensual']:.2f}")
    ...
    def _detect_outliers_iqr(self):
        ...
        IQR = Q3 - Q1
        outliers = self.df[(self.df['produccion_mwh'] < (Q1 - 1.5 * IQR)) | 
    ...
    def _calculate_volatility(self, dept):
        dept_data = self.df[self.df["departamento"] == dept]["produccion_mwh"]
        return float(dept_data.std() / dept_data.mean()) if dept_data.mean() > 0 else 0.0
    ...
    def _calculate_market_concentration(self):
        ...
        return {
            "hhi_index": float(hhi),
            "market_concentration": "Alto" if hhi > 0.25 else "Medio" if hhi > 0.15 else "Bajo"
        }
    ...
    def _calculate_tech_growth(self, tech):
        ...
        return {
            "average_growth_rate": float(growth_rates.mean()) if len(growth_rates) > 0 else 0.0,
            "trend": "creciente" if growth_rates.mean() > 0 else "decreciente"
        }

    def _analyze_geographic_distribution(self, tech):
        tech_data = self.df[self.df["tecnologia"] == tech]
        geo_dist = tech_data.groupby("departamento")["produccion_mwh"].sum().sort_values(ascending=False)
    ...
    def _analyze_climate_correlation(self, tech):
        ...
        return {
            ...
            "summer_winter_ratio": float(summer_avg / winter_avg) if tech == "Solar" and winter_avg > 0 else 1.0
        }

    def _calculate_tech_complementarity(self):
        if "Solar" in self.df["tecnologia"].values and "Eólica" in self.df["tecnologia"].values:
            solar_monthly = self.df[self.df["tecnologia"] == "Solar"].groupby("mes")["produccion_mwh"].mean()
            eolica_monthly = self.df[self.df["tecnologia"] == "Eólica"].groupby("mes")["produccion_mwh"].mean()
            correlation = float(solar_monthly.corr(eolica_monthly))
            return {
                "correlation": correlation,
                "complementarity": "alta" if correlation < -0.5 else "media" if correlation < 0.3 else "baja"
            }
    ...
    def _calculate_diversification_index(self):
        ...
        return {
            ...
            "level": "Alto" if diversification > 0.8 else "Medio" if diversification > 0.5 else "Bajo"
        }

    # Métodos adicionales para análisis temporal detallado
    def _analyze_monthly_patterns(self):
        monthly_stats = self.df.groupby("mes")["produccion_mwh"].agg(['mean', 'std', 'min', 'max', 'count'])
        return {str(k): {
    ...
    def _analyze_quarterly_patterns(self):
        quarterly_stats = self.df.groupby("trimestre")["produccion_mwh"].agg(['sum', 'mean', 'count'])
        return {str(k): {
            'total': float(v['sum']),
            'promedio': float(v['mean']),
            'registros': int(v['count'])
    ...
    def _analyze_yearly_patterns(self):
        yearly_stats = self.df.groupby("año")["produccion_mwh"].agg(['sum', 'mean', 'count'])
        return {str(k): {
            'total': float(v['sum']),
            'promedio': float(v['mean']),
            'registros': int(v['count'])
    ...
    def _analyze_time_trends(self):
        ...
        if "año" in self.df.columns:
            ...
            if len(yearly_production) > 1:
                trend_slope = (yearly_production.iloc[-1] - yearly_production.iloc[0]) / len(yearly_production)
                return {
    ...
    def _analyze_detailed_seasonality(self):
        ...
        return {
            "indices_estacionales": {str(k): float(v) for k, v in seasonal_index.items()},
    ...
    def _analyze_temporal_volatility(self):
        ...
        return {
            "volatilidad_por_mes": {str(k): float(v) for k, v in monthly_volatility.items()},
    ...
    def _calculate_gini(self, shares):
        ...
        return float((2 * np.sum((np.arange(1, n + 1) * sorted_shares))) / (n * cumsum[-1]) - (n + 1) / n)
    ...
    def _identify_potential_synergies(self):
        ...
        for dept in self.df["departamento"].unique():
            ...
            if len(tech_presence) == 1:  # Solo una tecnología
                missing_tech = [t for t in self.df["tecnologia"].unique() if t not in tech_presence.index]
                synergies.append({
                    "departamento": dept,
    ...
    def _identify_market_gaps(self):
        ...
        cross_matrix = pd.crosstab(self.df["departamento"], self.df["tecnologia"], 
                                  values=self.df["produccion_mwh"], aggfunc="sum").fillna(0)
        ...
        for dept in cross_matrix.index:
            for tech in cross_matrix.columns:
                if cross_matrix.loc[dept, tech] == 0:
                    ...
                    similar_depts_avg = cross_matrix[cross_matrix[tech] > 0][tech].mean()
                    gaps.append({
                        "departamento": dept,
                        "tecnologia": tech,
    ...
    def _identify_expansion_opportunities(self):
        ...
        dept_performance = self.df.groupby("departamento")["produccion_mwh"].agg(['sum', 'mean', 'count'])
        ...
        for dept in dept_performance.index:
            ...
            elif dept_stats['count'] < self.df.groupby("departamento").size().mean() * 0.8:
                opportunities.append({
                    "departamento": dept,
                    "tipo": "expansion_cobertura",
                    "razon": "baja_densidad_instalaciones",
                    "instalaciones_actuales": int(dept_stats['count']),
                    "potencial_instalaciones": int(self.df.groupby("departamento").size().mean())
                })
